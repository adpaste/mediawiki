...
class MediaWiki {
	...
	/**
	 * @param FauxRequest|WebRequest $request
	 * @return void
	 * @throws MWException
	 */
	private function setupTracing( $request ): void {
		$services = MediaWikiServices::getInstance();
		$tracer = $services->getTracer();

		$parentContext = $services->getTraceContextPropagator()
			->extract( $request->getAllHeaders() );
		...
		// In an HTTP request context, use the HTTP method as our root span name per Semantic Conventions,
		// since the path is very likely too high cardinality to be safely usable as a span name.
		// In a CLI context, the PHP script being executed serves as a natural root span name.
		...
		$context = $this->context;
		$tracerFactory = $services->getTracerFactory();
		register_shutdown_function( static function () use ( $context, $scope, $rootSpan, $tracerFactory ) {
			// Infer the span status from the HTTP response code if applicable, subject to Semantic Conventions.
			// That is, treat any 5xx as an ERROR and anything else as UNSET.
			$statusCode = http_response_code();
			$spanStatus = $statusCode >= 500 ? StatusCode::STATUS_ERROR : StatusCode::STATUS_UNSET;

			$rootSpan->setAttribute( TraceAttributes::HTTP_RESPONSE_STATUS_CODE, http_response_code() );
			$user = $context->getUser();
			if ( $user->isSafeToLoad() ) {
				// setting user.type here, as it can't be read easily at an earlier stage
				$rootSpan->setAttributes( [ "user.type" => $user->isAnon() ? "anon" : "authorized" ] );
			}

			// For redirect responses (3xx), record the Location header as a span attribute.
			// This is mainly to improve the introspection of redirect responses
			// generated by forms like Special:Upload, where the redirect target cannot be inferred
			// from just the request URL.
			if ( $statusCode > 300 && $statusCode < 400 ) {
				$location = $context->getRequest()->response()->getHeader( 'Location' );
				if ( $location !== null ) {
					$rootSpan->setAttribute( 'http.response.header.location', $location );
				}
			}

			$rootSpan->setStatus( $spanStatus );

			$rootSpan->end();
			$scope->detach();

			// Shut down all tracers after finishing request processing.
